...
8. **Reasoning models and chain-of-thought tagging**
   ...
   * On OpenRouter the effort is sent through its unified `reasoning` parameter, so it reaches every reasoning-capable model behind the aggregator. Models that only expose a token budget have the effort converted server-side; Gemini maps it to a thinking level.
