...
12. **Third-party integration (force other mods through RimLLM)**
    ...
    * Pure Harmony, all hooks fail-soft, and the framework implements none of RimTalk's interfaces: `AIClientFactory.GetAIClientAsync` (the single transport choke point) hands RimTalk a *sentinel* instance of its own `OpenAIClient`; the two non-generic funnels every request passes through (`OpenAIClient.GetChatCompletionAsync` and the private `StreamAsync`) are prefixed so that the sentinel instance — and only that instance — is redirected into RimLLM, while a player's own native client is left untouched. `RimTalkSettings.GetActiveConfig` is given a synthetic config only when RimTalk has none of its own, so a player who never configured RimTalk still gets dialogue. Not implementing `IAIClient` is deliberate: an interface implementation is a type-level reference that RimWorld resolves when loading the DLL, and it would make RimWorld reject the entire framework assembly whenever RimTalk is absent (method signatures and bodies are only resolved at JIT time, so those may mention RimTalk types). If RimTalk is not installed the hooks are never applied; if its API has drifted the hook fails to attach and the settings tab says so. If RimLLM has no usable provider at request time (empty fallback chain, missing keys) the request falls back to RimTalk's native path with a throttled warning — offline/Player2 users who flip the switch by mistake are not left silent.
