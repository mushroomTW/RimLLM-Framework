...
namespace RimLLM_Framework.Mod
{
    ...
    /// </summary>
    public class ArchotechNexusSettings : ModSettings
    {
        /// <summary>
        /// API 供應商的 Fallback Chain 順序。
        /// </summary>
        public List<string> FallbackChain = new List<string> { "Gemini", "DeepSeek", "OpenAI", "OpenAICompatible" };
        ...
        {
            ["OpenAI"] = new List<string> { "gpt-4o", "gpt-4o-mini" },
            ["Gemini"] = new List<string> { "gemini-2.5-pro", "gemini-2.5-flash" },
            ["DeepSeek"] = new List<string> { "deepseek-chat" },
            ["OpenAICompatible"] = new List<string> { "default" }
        };
            ...
            else if (Scribe.mode == LoadSaveMode.LoadingVars)
            {
                ...
                if (!string.IsNullOrEmpty(jsonStr))
                {
                    try
                    {
                        ...
                        if (dto != null)
                        {
                            if (dto.FallbackChain != null) this.FallbackChain = dto.FallbackChain;
                    ...
                    {
                        Log.Error($"[ArchotechNexus] 載入設定失敗: {ex.Message}");
                    }
                }
            }
        }
