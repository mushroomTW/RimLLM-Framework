...
namespace RimLLM_Framework.Core
{
    ...
    /// </summary>
    public class ArchotechDispatcher : MonoBehaviour
    {
        private static ArchotechDispatcher _instance;
        ...
        /// </summary>
        public static ArchotechDispatcher Instance
        {
            get
            {
                if (_instance == null)
                {
                    GameObject go = new GameObject("ArchotechDispatcher");
                    _instance = go.AddComponent<ArchotechDispatcher>();
        ...
        private void Update()
        {
            ...
            while (ExecutionQueue.TryDequeue(out Action action))
            {
                ...
                {
                    ArchotechLog.Error($"[ArchotechNexus] 主線程分發 Callback 執行失敗: {ex.Message}\n{ex.StackTrace}");
                }
            }
        }
